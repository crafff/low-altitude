...
The figure shows 19 evaluations from 0 through 400 completed episodes from one training trajectory on the same 12 predeclared development cases. No held-out results, independent training replicates, smoothing, error bars or significance claims are included. The interrupted run reached 65 episodes but has no evaluation at 65; the next resumed job supplied that evaluation.
