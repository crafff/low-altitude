...
def main():
    ...
    summary = dict(all_checks_passed=True,
        ...
        paired_cases=[dict(seed=case['seed'], corridor_count=case['corridor_count'],
            initial=helpers.metrics(dev[0]['cases'][i]['sample']),
            at256=helpers.metrics(dev[3]['cases'][i]['sample']),
            best=helpers.metrics(selected['cases'][i]['sample']),
            final=helpers.metrics(dev[-1]['cases'][i]['sample']))
