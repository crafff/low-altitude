...
def main():
    ...
    assert new['configs']['environment_parts']['scenario']['per_flight_timeout_seconds'] == 2400.
    assert new['configs']['environment']['exit_width_tolerance_m'] == .0001
    assert new['configs']['training']['training_scenario_seed_start'] == 9600000
    assert new['configs']['training']['sampling_seed'] == 9610000
    assert new['configs']['training']['shuffle_seed'] == 9620000
    for name, digest in new['versions']['source_sha256'].items():
    ...
    assert [d['completed_episodes'] for d in old_dev] == [0, 64]
    assert [d['completed_episodes'] for d in new_dev] == [64, 128, 256]
    ...
    for index, batch in enumerate(training):
        ...
        assert batch['scenario_seeds'] == list(range(9600000 + index * 4, 9600004 + index * 4))
        ...
        for e in batch['episodes']:
            i = e['episode_index']
            assert (e['scenario_seed'], e['action_seed'], e['global_seed'], e['policy_version']) == (9600000 + i, 9610000 + i, 950001 + i, index)
    ...
    summary = dict(all_checks_passed=True, scope='new2400s lineage; one initialization seed; four seen DEV checkpoints; descriptive, not held-out efficacy',
                   ...
                   best_selection_key=selected['selection_key'], final_vs_initial=change(development[0]), final_vs_64=change(development[1]),
    ...
    reference = read(args.frozen_reference)['groups']
    assert reference['new2400/nr']['aggregate'] == old_dev[0]['aggregate']['nr']
    summary['frozen_old256_new2400'] = metrics(reference['new2400/256']['aggregate'])
    summary['final_vs_frozen_old256'] = change(summary['frozen_old256_new2400'])
    ...
    for ax, key, label in zip(axes, ('completed_fraction', 'nmac_seconds_per_flight_hour', 'lowc_seconds_per_flight_hour'), ('Mission completion (%)', 'NMAC pair-seconds / flight-hour', 'LoWC pair-seconds / flight-hour')):
        ...
        if key == 'completed_fraction':
            ...
            ax.set_ylim(min(85., min(d['completed_fraction'] for d in development)*100-2.), 103)
