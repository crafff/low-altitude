...
| CAP航点切换不等于横移完成；F020第三代628.25s仅侧误差超过2m，满足所有条件时正常释放 | 分别记录CAP、普通航点锁内和释放后状态；先查实际误差/判据，勿直接放宽容差或称永久锁 | [原100模型3188行精确重放与5回归](../reports/lane-completion-f020-20260905/README.md) |
...
- 终止容差或期限改变后，必须重新运行完整交通。0.1mm出口校准使冻结256完成234→291；再把模拟年龄1200→2400s，53慢机全部到达，另4出口失败也转到达，最终348/360。NMAC累计5332.5→5983且每FH80.20→83.58，不能把完成增加或归一化下降自动当学习收益。共有到达样本与新增慢机应分开报航时；数值容差是显式工程接受口径，不冒充IEEE误差证明。[108次冻结对照/3240逐机核算](../reports/terminal-calibration-20260906/README.md)。
