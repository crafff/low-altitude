...
用户“好”接受先冻结新场景验证再独立训练种子。先执行有限评价闭环：reports/frozen-new24-20260906/protocol.json固定24场景seed10730001–10730024，3/4/5航道各8，原生成分布30架/2400s环境不变，不按任何结果筛选。冻结best1024 SHA76453bca0a5ad54aa5d9ece85268b2af586616ccfaab92037f265283d6588a44，对照相同seed950001未训练初始化和NR；同case动作seed810000+场景seed。非DEV选择、不更新参数、不比较其他checkpoint择优。单CPU14/nice15/idleIO/lab内1140外1200秒，核算另限120秒。先完成这一步后据结果安排独立种子重复，不继承旧10h，不操作其他实验。

评价前确定归约：全24聚合无序风险秒/FH、任务完成率，NR与未训练双对照；按3/4/5航道分层、整场景配对bootstrap2000次seed10739999给条件于单训练seed的描述性95%区间，不作多种子稳定性保证。未训练参数与原pilot64 best0逐张量核对。首次launcher缺--config在启动前拒绝，补显式环境配置后启动，未绕过sandbox。


### 2026-09-06 04:44 UTC：新24完成，泛化学习收益未确认

评价job20260906T042924Z-frozen-new24-358e29a3于04:41:00.855 succeeded，24×3场景/2160计划架次，内694.361363/外696.125092秒；核算job20260906T044202Z-frozen-new24-audit-83be2b7c于04:42:06.723 succeeded、4.245127秒。总监督700.370219s，全部负载结束，未启动独立种子长训练。

[报告和精确记录](../reports/frozen-new24-20260906/README.md)：NR/未训练/冻结到达720/699/706（各720计划、均0超时），NMAC率184.810999/80.356171/89.336653、LoWC732.338129/434.825751/423.194845秒每FH。冻结相对未训练NMAC+11.175846%、LoWC−2.674843%，完成+7；原始NMAC11998.75→12349.75 pair-s、FH149.319583→138.238333，不能把全部率增幅称原始暴露增长。对初始9/24场景NMAC下降、14/24 LoWC下降。分层整场景配对bootstrap2000 seed10739999的95%区间NMAC[−2.338026%,24.330510%]，LoWC[−8.207394%,2.394392%]，均跨零；条件于单训练种子，一次场景动作采样，不证明普遍退化或过拟合，也未确认泛化学习收益。相对NR显著数值差距不能全算学习，因为初始已好于NR。

核算通过：24声明seed/航道、人口/终止、动作seed、风险/FH/聚合；原始初始化与pilot64 best0逐张量精确；评价本体源码/环境/PPO身份与checkpoint相同，模型/文件始终不变。13份560194字节源副本核对，PNG已目视。宽度原始冻结658架/97892.75秒、最大375.756714米、高度0，用户搁置过滤器前置未变。04:29:13及04:42:07两保护PID仍原ticks，GPU20886/3162MiB，无干预。单CPU14/lab/nice15/idleIO、无GPU，无科学核心变更，无子代理。

下一步建议新增两个独立训练seed各自1024场景并对应未训练对照，原DEV选模型，先明确随机种子变化范围与时间预算再执行，不按本次24调参。24已被查看，后续是固定外部评价面板；若调参需另留确认集。NOW更新，按既有授权提交push并由root通知。


## 2026-09-06：两个独立训练seed重复

用户“好”接受独立种子重复，按先前建议新增两个各1024。预先固定reports/independent-seeds-20260906/protocol.json：seed950002/950003；训练场景10800000/10900000+index，采样10810000/10910000+index，shuffle10820000/10920000。完整随机流程复现，不是仅初始化消融。原12DEV、每128评价、每16保存、≥95%完成后NMAC/LoWC原顺序选best；训练结束后固定已见external24逐seed比较原初始化和DEV-best，不利用external选择/调参。原seed64额外pilot未曾成为best，曲线比较共同128步点。科学算法/环境/超参/源代码不变，仅新配置种子、目标和运行预算及说明。

两个训练依次单lab，内5400外5460秒各；外部评价合计2400秒，独立核算120秒，总监督预算13440秒（224分钟）为本轮上限，预计2.5–3小时；按实测可提前结束，不继承旧10h。四CPU12–15/单线程PPO/nice15/idleIO，无GPU，不操作保护两实验。root本地无子代理。

用户要求两个任务同时执行、多CPU。已按明确请求切换单launcher内双trainer，各4核：seed2 CPU12–15，seed3 CPU8–11（lscpu确认8个不同物理核）。原自有launcher3181848精确cmdline匹配后SIGTERM，04:48:26.385结束，reason=supervisor_signal/143，监督55.308123s；当时仅初始化评价、无training.jsonl，pre-training latest保留并将strict resume，不操作保护PID3059945/3062529。中断记录不标成功。新train_pair.py仅同一sandbox内Popen两独立trainer，各自LAB_RUN_DIR/TMPDIR/log，无嵌套launcher/agent框架；共享源码只读，子进程同组受外launcher回收。算法不变，seed3唯一执行设置变更为CPU8–11。单新job外5460s/各trainer内5400，原总预算扣中断耗时，预计并行训练+评价约1.5–2小时。

并行初步实测：04:54:30两trainer亲和性12–15/8–11，保护PID原ticks，GPU20886/3162MiB、MemAvailable59403360kB。初始DEV种子2到达352/NMAC90.067801，种子3到达349/NMAC80.760291。128评价分别349/88.801140/463.382730与353/78.751736/397.310214（到达/NMAC/LoWC每FH），继续不调参。每四场景早期约13–14秒，单种子速度接近原单任务。

256 DEV：seed2到达351/360、NMAC78.540229、LoWC439.263589；seed3到达349/360、NMAC83.348787、LoWC418.150680。种子2较初始改善，种子3相对128回退；两者仍按固定预算继续，不调参或读取external模型表现。

384 DEV：seed2到达349、NMAC74.769511/LoWC389.070341；seed3到达352、NMAC72.149617/LoWC383.784121。05:27:27保护PID原ticks，GPU20886/3162MiB、MemAvailable59392316kB。

512 DEV分别seed2 348/77.323058/397.857206、seed3 352/74.534986/401.149331；640分别347/70.634517/402.769650与349/85.839483/392.574212。seed2 best640，seed3 best384；同算法不同随机流程差异增大，未调参。05:37:08保护PID原ticks/GPU20886/3162MiB。

两个训练完成：单launcher job20260906T044931Z-independent-pair-1024-6d895f1e于06:06:53.613 succeeded，监督4642.586361秒（77.38min）。seed2内4640.637495s、1024完成、best1024；seed3内4558.527672s、1024完成、best896。seed2最终DEV354/360、NMAC66.867701/LoWC382.731426；seed3最终355/360、76.649530/380.824614，保留best896。无超预算或额外训练，22份11799777字节备份含中断作业。随后单lab内CPU14/10并行外部评价，仅各自初始和DEV-best，复用固定场景/原NR。


### 2026-09-06 06:20 UTC：并行独立重复和评价全部完成

[完整报告、协议、精确记录](../reports/independent-seeds-20260906/README.md)。两个新增种子训练均1024/256更新批，样本4509725/4472743、Adam步70591/70017，平均批13.386781/13.117065s。单lab双trainer成功（04:49:31.027→06:06:53.613、监督4642.586361s），seed2 best1024、seed3 best896，latest均1024。原单任务因用户要求并行仅自有launcher中断55.308123s，初始恢复点和failed/supervisor_signal记录保留；无训练丢失或他人进程操作。

外部job20260906T060811Z-independent-pair-external24-6e71360a（06:08:11.296→06:16:40.341、外509.045169s）单lab内CPU14/10双评价，各seed24×2，冻结DEV-best及各自未训练，固定场景/动作种子，NR原始case复用，不择优外部模型。seed2初始699→704到达、NMAC85.017597→73.364734（−13.706413%）、LoWC428.727726→382.573667（−10.765354%）；seed3 702→697、87.431466→83.730440（−4.233059%）、442.127663→413.982895（−6.365756%）。均720计划/0超时，未完成是路线耗尽/出口。原seed1 NMAC+11.175846%/LoWC−2.674843%/到达+7；合计三seed两降一升，NMAC等权平均相对变化−2.254542%、LoWC−6.601984%，仅描述、不称收敛或稳定泛化。没有使用外部结果挑最好seed作为唯一结论。

原始暴露seed2 NMAC12782.75→10125.5、LoWC64461→52801.25 pair-s、FH150.354167→138.015903；seed3 13096.25→11620、66225.75→57452、149.788750→138.778681。不能忽略seed3到达减少5架，NR差距也不是全部学习收益。

独立audit job20260906T061739Z-independent-pair-audit-64fefd5d（06:17:39.179→06:17:43.348、4.168989s）成功。核对配置不变范围、源版本、1024连续种子/人口/256批策略身份/样本与PPO遍历、9点DEV聚合/选择键、latest内嵌best和独立best全模型/Adam/RNG、有限权重、外部24声明/NR/聚合及checkpoint来源；全部断言通过，root目视图。40份12399686字节产物逐源副本核对，四新模型与原中断初始化恢复点保存。原科学核心/环境未改，仅两个随机seed配置及单lab编排/核算/报告，无子代理。

总监督55.308123+4642.586361+509.045169+4.168989=5211.108642s（86.85min），低于13440s，不继承旧预算或自动延长。所有负载结束。保护PID3059945/3062529在04:47/04:54/05:07/05:17/05:27/05:37/05:47/05:55/06:18均原ticks348614026/348620130，末GPU20888/3160MiB、MemAvailable63738260kB。未操作其信号/优先级/环境；抽样存活不保证瞬时吞吐不变。host-parallel-1还记录临时诊断进程3194126，并非训练worker，实际trainer3185439/3185440亲和12–15/8–11。

NOW更新。下一步先现有训练/DEV记录作失败归因、奖励和NMAC对应/动作分布，再定小对照，不直接只扩最好seed。固定external24已见，不调参后再称全新测试；用户搁置越界前置不变。按既有授权准备私有分支提交push和root完成通知。

收尾：40份源/副本/暂存blob精确，四编排/核算脚本及两个训练配置与实际快照一致，doctor无issues/local_missing、科学src diff空。暂存空白检查仅两原始BlueSky并行训练日志各10处行尾空格，保持证据原字节；排除这两原始日志后其余全部通过。GitHub仍PRIVATE，按既有授权提交push及root通知。
