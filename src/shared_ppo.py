"""CPU shared-policy attention and PPO; collection/checkpoints live in the runner.
...
def _config(cfg: Mapping[str, Any]) -> dict[str, Any]:
    result = {**DEFAULTS, **cfg}
    for key, expected in (("device", "cpu"), ("torch_num_threads", 1),
                          ("own_dim", 7), ("intruder_dim", 10),
                          ("action_dim", 60)):
...
def collate(observations: Sequence[Mapping[str, Any]]) -> tuple[torch.Tensor, ...]:
    """Copy observations into CPU tensors in the model's forward argument order.
    ...
    return (torch.from_numpy(np.stack(own_rows)), torch.from_numpy(padded),
            torch.from_numpy(present), torch.from_numpy(np.stack(mask_rows)))


class SharedActorCritic(nn.Module):
    """One current-neighbor query and a shared actor/critic backbone, CPU only."""
    ...
    def forward(self, own: torch.Tensor, intruders: torch.Tensor,
        ...
        if any(t.device.type != "cpu" for t in tensors) or self.query.weight.device.type != "cpu":
            raise ValueError("SharedActorCritic requires CPU tensors and parameters")
...
def _sample_tensor(samples: Sequence[Mapping[str, Any]], field: str) -> torch.Tensor:
    result = []
    ...
    tensor = torch.tensor(result, dtype=torch.float32, device="cpu")
...
def update(model: SharedActorCritic, optimizer: torch.optim.Optimizer,
    ...
    actions = torch.tensor(chosen, dtype=torch.long, device="cpu")
    ...
    old_log_prob = _sample_tensor(samples, "old_log_prob")
    if bool((old_log_prob > 1e-6).any()):
        raise ValueError("stored action log probabilities cannot be positive")
    advantage = _sample_tensor(samples, "advantage")
    returns = _sample_tensor(samples, "return")
