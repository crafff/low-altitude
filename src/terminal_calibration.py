...
def main():
    ...
    parser.add_argument('--only-seed', type=int, choices=range(53001, 53013),
                        help='Explicit single seen DEV case for bounded completion/replay')
    ...
    if args.only_seed is not None:
        scenarios = [s for s in scenarios if s['seed'] == args.only_seed]
        if not scenarios:
            raise ValueError('Selected seed is outside case-limit')
            ...
            for arm, tolerance, timeout, policies in ARMS:
                ...
                for policy in policies:
                    ...
                    row = dict(arm=arm, policy=policy, seed=scenario['seed'],
                        ...
                        summary=summary, flights=env.details(), events=copy.deepcopy(env.tracker.events),
                        prefix_before_1200_sha256=env.prefix.hexdigest())
                    rows.append(row)
